module Spring exposing (Model, Msg, main)
...
dragPtStyle : DragPt.Style
dragPtStyle =
    let
        (DragPt.Style defStyl) =
            DragPt.defaultStyle
    in
    DragPt.Style { defStyl | colorStroke = Color.white }


init : Model
init =
    Model
        { pt1 = DragPt.init dragPtStyle (Math.p2 50 300)
        , pt2 = DragPt.init dragPtStyle (Math.p2 550 300)
        }
